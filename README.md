...
Some explanation is required.  A challenge with making pixel by pixel comparisons between x-rays of lungs is we cannot line up x-rays of lungs with different kinds of disease and healthy lungs.  In most cases, x-rays of lungs with pneumonia and healthy lungs are from different patients with different sizes of ribs and lungs where the angles are different, scopes are different, and shadings are different.  Making comparisons would not be possible unless we are able to generate x-rays using GANs where the only differences are embedded labels.  

As we observed in https://github.com/tvtaerum/cGANs_housekeeping and https://github.com/tvtaerum/xrays_housekeeping, we are able to generate images which are based on the same weights modified only by an embedding label ("attractive male" vs "attractive female with high cheeks bones") or ("x-rays of healthy lungs" vs "viral pneumonia" vs "bacterial pneumonia").  What happens when we apply the same processes to x-ray images of healthy lungs and those with bacterial and viral pneumonia. Are the predictions sufficiently strong that we can visually distinguish between healthy, viral pneumonia, and bacterial pneumonia based on generated images?    
